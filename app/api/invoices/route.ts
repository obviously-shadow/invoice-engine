...
export async function POST(request: Request) {
  try {
    ...
    const insertInvoice = db.prepare('INSERT INTO invoices (client_name, client_email, client_address, token, status, tax_rate, notes, is_tbd, display_number, due_date, deposit_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    ...
    const transaction = db.transaction(() => {
      const info = insertInvoice.run(client_name || 'Valued Client', client_email, client_address, token, 'draft', tax_rate, notes || '', is_tbd ? 1 : 0, nextDisplayNum, due_date || '', parseFloat(deposit_amount) || 0);
